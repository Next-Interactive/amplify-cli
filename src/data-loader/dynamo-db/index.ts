...
export class DynamoDBDataLoader implements AmplifyAppSyncSimulatorDataLoader {
  ...
  constructor(private ddbConfig: DynamoDBLoaderConfig) {
    ...
    if (!tableName || !endpoint) {
      throw new Error(
        `Invalid DynamoDBConfig ${JSON.stringify(ddbConfig, null, 4)}`
      );
    }
  ...
  private async batchGetItem(payload: {
    tables: {
      [tableName: string]: { keys: DynamoDB.Key[]; consistentRead?: boolean };
    };
  }) {
    ...
    return {
      data: mapTableObject(Responses, items =>
        items.map(item => unmarshall(item))
      ),
      unprocessedKeys: mapTableObject(UnprocessedKeys || {}, request =>
        request.Keys.map(key => unmarshall(key))
      )
    };
  }
  ...
  private async updateItem(payload) {
    ...
    const { Attributes: updated } = await this.client
      .updateItem(params)
      .promise();
